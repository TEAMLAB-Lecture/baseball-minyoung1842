...
def is_between_100_and_999(user_input_number):
    ...
    result = True if 100 <= temp < 1000 else False
...
def is_duplicated_number(three_digit):
    ...
    # ===Modify codes below=============
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당
    # a, b, c = map(int,three_digit)
    # result = False if a != b and b != c and a != c else True

    # # ==================================
    # return result
    for digit in str(three_digit):
        if str(three_digit).count(digit) > 1: 
            return True
    return False
...
def get_not_duplicated_three_digit_number():
    ...
    while x == "0": 
        x = str(randrange(10))
    while len(x)<3:
        temp = str(randrange(10))
        if x[0] != temp and x[-1] != temp: x += temp
    result = int(x)
    # ==================================
    return result
...
def is_yes(one_more_input):
    ...
    result = True if temp in ['y', 'yes'] else False
...
def is_no(one_more_input):
    ...
    temp = one_more_input.lower()
    result = True if temp in ['n', 'no'] else False
    # ==================================
    return result
